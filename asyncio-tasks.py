   Link: author: About these documents
   Link: index: Index
   Link: search: Search
   Link: copyright: Copyright
   Link: next: Streams
   Link: prev: asyncio — Asynchronous I/O
   Link: canonical
   [ ]  Logo
...
Sleeping¶
   ...
   coroutine asyncio.sleep(delay, result=None)¶
           ...
           blocking the event loop for the full duration of the function
           call.
           ...
           Example of coroutine displaying the current date every second for
           5 seconds:
...
Running Tasks Concurrently¶
   ...
   awaitable asyncio.gather(*aws, return_exceptions=False)¶
           ...
           gather(). Other awaitables in the aws sequence won’t be cancelled
 ...
 #     [2, 6, 24]
           ...
           instance, gather can be marked done after propagating an exception
           to the caller, therefore, calling gather.cancel() after catching
           an exception (raised by one of the awaitables) from gather won’t
           cancel any other awaitables.
           ...
           no positional arguments are provided or not all positional
           arguments are Future-like objects and there is no running event
           loop.
...
Shielding From Cancellation¶
 ...
 res = await something()
           ...
           something(), the cancellation did not happen. Although its caller
           is still cancelled, so the “await” expression still raises a
           CancelledError.
...
Waiting Primitives¶
   ...
   coroutine asyncio.wait(aws, *, timeout=None, return_when=ALL_COMPLETED)¶
           ...
           return_when indicates when this function should return. It must be
           one of the following constants:

           Constant        Description                                        
           FIRST_COMPLETED The function will return when any future finishes  
                           or is cancelled.                                   
                           The function will return when any future finishes  
           FIRST_EXCEPTION by raising an exception. If no future raises an    
                           exception then it is equivalent to ALL_COMPLETED.  
           ALL_COMPLETED   The function will return when all futures finish   
                           or are cancelled.                                  
           ...
           coroutines objects to wait() directly is deprecated as it leads to
           confusing behavior.
 ...
 if task in done:
   ...
   asyncio.as_completed(aws, *, timeout=None)¶
           ...
           Run awaitable objects in the aws iterable concurrently. Return an
           iterator of coroutines. Each coroutine returned can be awaited to
           get the earliest next result from the iterable of the remaining
           awaitables.
...
Running in Threads¶
   ...
   coroutine asyncio.to_thread(func, /, *args, **kwargs)¶
           ...
           executing IO-bound functions/methods that would otherwise block
           the event loop if they were ran in the main thread. For example:
 ...
 # finished main at 19:50:54
           ...
           event loop for its duration, resulting in an additional 1 second
...
Introspection¶
   ...
   asyncio.current_task(loop=None)¶
           ...
           Return the currently running Task instance, or None if no task is
           running.
   ...
   asyncio.all_tasks(loop=None)¶
           ...
           If loop is None, get_running_loop() is used for getting current
           loop.
...
Task Object¶
   ...
   class asyncio.Task(coro, *, loop=None, name=None)¶
           ...
           A Future-like object that runs a Python coroutine. Not
           thread-safe.
           ...
           Event loops use cooperative scheduling: an event loop runs one
           Task at a time. While a Task awaits for the completion of a
           Future, the event loop runs other Tasks, callbacks, or performs IO
           operations.

           Use the high-level asyncio.create_task() function to create Tasks,
           or the low-level loop.create_task() or ensure_future() functions.
           Manual instantiation of Tasks is discouraged.

           To cancel a running Task use the cancel() method. Calling it will
           cause the Task to throw a CancelledError exception into the
           ...
           loop is not specified and there is no running event loop.
                ...
                cancel(msg=None)¶
                        ...
                        This arranges for a CancelledError exception to be
                        thrown into the wrapped coroutine on the next cycle
                        of the event loop.

                        The coroutine then has a chance to clean up or even
                        deny the request by suppressing the exception with a
                        try … … except CancelledError … finally block.
                        Therefore, unlike Future.cancel(), Task.cancel() does
                        not guarantee that the Task will be cancelled,
                        although suppressing cancellation completely is not
                        common and is actively discouraged.

                        Changed in version 3.9: Added the msg parameter.

                        The following example illustrates how coroutines can
 ...
 #     main(): cancel_me is cancelled now
                ...
                cancelled()¶
                        ...
                        The Task is cancelled when the cancellation was
                        requested with cancel() and the wrapped coroutine
                        propagated the CancelledError exception thrown into
                        it.
                        ...
                        A Task is done when the wrapped coroutine either
                        returned a value, raised an exception, or the Task
                        was cancelled.
                ...
                result()¶
                        ...
                        If the Task is done, the result of the wrapped
                        coroutine is returned (or if the coroutine raised an
                        exception, that exception is re-raised.)

                        If the Task has been cancelled, this method raises a
                        CancelledError exception.

                        If the Task’s result isn’t yet available, this method
                        raises a InvalidStateError exception.

                exception()¶

                        Return the exception of the Task.

                        If the wrapped coroutine raised an exception that
                        exception is returned. If the wrapped coroutine
                        returned normally this method returns None.
                ...
                add_done_callback(callback, *, context=None)¶
                        ...
                        This method should only be used in low-level
                        callback-based code.
                ...
                get_stack(*, limit=None)¶
                        ...
                        If the wrapped coroutine is not done, this returns
                        the stack where it is suspended. If the coroutine has
                        completed successfully or was cancelled, this returns
                        an empty list. If the coroutine was terminated by an
                        ...
                        The optional limit argument sets the maximum number
                        of frames to return; by default all available frames
                        are returned. The ordering of the returned list
                        differs depending on whether a stack or a traceback
                        is returned: the newest frames of a stack are
                        returned, but the oldest frames of a traceback are
                        returned. (This matches the behavior of the traceback
                        module.)
                ...
                print_stack(*, limit=None, file=None)¶
                        ...
                        This produces output similar to that of the traceback
                        module for the frames retrieved by get_stack().

                        The limit argument is passed to get_stack() directly.

                        The file argument is an I/O stream to which the
                        output is written; by default output is written to
                        sys.stderr.
                ...
                get_name()¶
                        ...
                        If no name has been explicitly assigned to the Task,
                        the default asyncio Task implementation generates a
                ...
                set_name(value)¶
                        ...
                        In the default Task implementation, the name will be
                        visible in the repr() output of a task object.
...
Generator-based Coroutines¶
 ...
 async def main():
  ...
  Navigation
   ...
   Last updated on Oct 02, 2022. Found a bug?
