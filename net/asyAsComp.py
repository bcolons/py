...
async def main():
    # Schedule three calls *concurrently*: larger nums require less sleep / so time
    ...
    for coro in asyncio.as_completed(iterfun):# each of the three added to event loop 
        print("someone's DONE!!")
        earliest_result=await coro #the first coro  is the first coro to return from its last await...only to to be await'd on again ...which returns the result.
        # await returns the paused execution stack with a callable/awaitable( IP->now_below_await) it doesnt not unsuspend anything leaving that at the caller's discretion
        # re-awaiting a coro with no remaining awaits *does* unpause the awaitable and resume normal/blocking execution (somehow it knows to await-return after the last await and to block-whatev-return for the sbsq await (which youd always need to do)
