...
class LinkL(): #ordered described as 2prior-impl below
    ...
    def dump_to_json(self):
        with open('primes.json','w') as fs:
            json.dump(dump_to_list(self),fs,indent=92)

    def r_getitem__(self,i):
        '''(Really slow) impl of Iterable, better to use Iterator/__iter__'''
        curref=self.head
        cur=0
...
def dump_to_list(iterable,lst=None):
    '''Helper, calls __str__(elem) for any iterable '''
    if not lst:
        lst=[] 
    for elem in iterable:
