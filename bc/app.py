...
def ppp_recur_str(obj='teststring....[{(4,5,6),(7,8),{a,b},},abf]',ind=0,): # like with inspect.getmembers(..) as [(input attr_name, attr_val
...
def rpppt(obj,ind=0):
    '''Ppprint to follow __repr__ format o/w same as above, should be later useable as serialize()/write_state()/etc...'''
    print(' '*4*ind+' t: ',type(obj),' o: ',repr(obj)) # main output, nested data is sbsq ppp(..)'d 
    if(isdct(obj)): # special case Z is-a-dict, fixmeZ: keys are allowed to be any hashable, bc_ here only primitives
        for eachkey in dct:
            rpppt(dct[eachkey],ind+1) # again fixmeZ, we leave the keys un-descended
    elif(isobj(obj)): # special case Y, has-a-dict 
        rpppt(obj.__dict__,ind+1)
    elif(not isstr(obj)): # str is iterable but not what we want to do here
        try: # is iterable ? pprettyp each one.
            for each in obj:
                rpppt(each,ind+1)
...
def isobj(inp): # silly test but not sure how else to do it, everthing else not None might be an obj?
    try:
...
def aord(inp): 
    '''Map A-Za-z onto 1-52'''
    o=ord(inp)
    if o < 91:
        return o - 64 # gives 1 for A, 2 for B, etc...
    else:   
        return o - 70  # gives 27 for a, 28 for B, etc...
def achr(inp):
    '''Map 1-52 onto A-Za-z'''
    if inp < 27:
        return chr(inp+64) # gives A for 1, B for 2 
    else:
        return chr(inp+96-26) # gives a for 27. b for 28
