import math
class RainTable:
    '''Class is a multi-threaded (threadsafe) server singleton for primality queries.
    Reads are completed, a read-lock is released, a single writer write-locks the array and may commence update..
    Locking system has five states:  
        i. accepting new readers, rlock is incremented to number of current readers (wlock==0)
        ii. writer asked for wlock, no more new readers, rlock will decrement as readers complete
        iii. rlock == 0, no more readers, 
        iv. if rlock==0 then if a waiting writer, wlock set from 0 to 1
        v. write is complete, wlock set from 1 to 0, rlock may be incremented anew, or another writer may set wlock from 0 to 1
        rlock=threading.Semaphore(max=9999) rl.acquire() #dec a max_ct, when Semaphore._value==max_ct 'rl is zero/unlocked'
        wlcok=threading.Lock() wl.acquire(blocking=True)
    where contention among writers, lowest prime should be allowed to work ...will return soonest to allow readers back in
    '''
